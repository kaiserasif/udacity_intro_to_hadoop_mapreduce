...
def mapper():
    ...
    for line in reader:
	if not isInteger(line[0]): continue # header row
        
	if len(line) == 5: # student info
	    writer.writerow(line[:1] + ["A"] + line[1:])

	elif len(line) == 19: # forum post
	    writer.writerow([line[3], "B"] + line[:4] + line[5:10])
